...
We already created these covariates in the first workshop (`Dist_Quoddy_sq`, `Dist_Brier_sq`). We can now define our components and formulae. Unlike before, we need to define a single components object that contains **all** the required components for both the whale watch **and** survey sightings. 
...
Brier_hn$Dist_Brier_sq <- exp(-5*Brier_hn$Dist_Brier_sq)
ggplot() + gg(Brier_hn) + gg(Sightings_Brier_nodup,color='green') + gg(Domain)
...
Looking at the third plot, we see that -150 could be a reasonable value for both `Dist_Quoddy_sq` and `Dist_Brier_sq`. We use this to define an informative prior on these parameters in the components stage next. We will put a Normal distribution prior, centered at -150, with standard deviation equal to 25 (equivalently a precision of 0.0016).
...
cmp_WW_Combined1 <- ~ 
  ...
  Dist_Quoddy_sq(main=Dist_Quoddy_sq, model='linear', 
                 mean.linear=-150, prec.linear=0.0016) + # half normal shape
...
Note that we also include log_Depth, as it was found to be 'significant' in the previous workshop.
...
Notice the lack of `samplers` used in defining the likelihoods for the whale watch data. This is because we are assuming that the whole domain can be sampled, but with the actual sampling intensity governed by the half-norm function of distance from port. Alternatively, we could specify `samplers = Domain`. This would automatically exclude any values that lie on land. We choose not to do this as it can slow down the computation dramatically for complex domains and because we have manually fixed a large value of distance on land, achieving the same effect.

We can now fit the model using `bru`. It can be very slow to fit, so we suggest you load the pre-compiled file yourself using the code below:

```{r 'bruWW1', warning=FALSE, cache=TRUE}
# Slow to fit, so we load a pre-compiled file instead
fit_combined1 <- bru(fit2_like, fit_WW_B1, fit_WW_Q1, components = cmp_WW_Combined1, options = list(bru_initial = list(Brier_par = -150,
                                                   Quoddy_par = -150)))
...
```{r, eval=F}
fit_combined1 <- bru(fit2_like, fit_WW_B1, fit_WW_Q1, components = cmp_WW_Combined1, options = bru_options(bru_initial=readRDS('./Model_Files/states_combined1.rds'),
            bru_max_iter = 1))
```
...
fit_combined1$dic$dic #DIC is 21296 - new benchmark can't compare with previous models
summary(fit_combined1) # WAIC is 21503
```
...
We estimate that in total, the effort from the Brier port is around ~0.5 times greater than that from the Quoddy port. The effort is defined relative to the survey effort. This finding will be called into question later.
...
#compute the indices of the pixels that lie < 20km of nearest trackline
restricted_ind <-
...
Note that the previous model estimated Brier's effort surface to be only ~0.5 times bigger! This is unrealistic... Instead, in model 2, we will fix the total effort from Brier equal to 3.22 times that of Quoddy. To achieve this, we will replace the contrast term (`Diff_...`) with a log(3.22) offset to the Brier linear predictor. This is the reason we defined the model in terms of a contrast!
