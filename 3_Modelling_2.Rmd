...
In this final tutorial, we will learn how to jointly model presence-only opportunistic sightings with survey data within the `inlabru` package. We will demonstrate how to fit the models and use them to predict both the spatial whale intensity and the total number of groups. Finally, we will compare the models' predictive performances on the heldout test data.
...
*Fitting these joint models can take a while. For the purposes of this workshop, we have made all the pre-fitted model files available for use via the `readRDS` function. Please ignore any messages regarding inlabru object version 2.2.5. This message occurs because we fit the models using an older version of inlabru.*
...
The first equation is explained by the following log rule: 'the log of a product equals the sum of the logs'. For $i =$ Brier, $\alpha_{i,0}$ is split into two terms: the Quoddy intercept and the difference/contrast term. We do not need to update the formula defined in the previous workshop for the survey data. In case you forgot, here it is:
...
Notice the polygonal `samplers` used in defining the likelihoods for the whale-watch data. This is because we are assuming that the whale-watch vessels can only travel on water (defined by `Domain`). In fact, it is this argument that restricts the computation to the original study region `Domain` and thus eliminates any influence that the extended and buffered covariates may have had on inference! 
...
By adding more data, we can expect to gain power to detect species-environmental trends. However, estimated trends from this joint model framework should be treated with caution. The causal DAG tells us that the estimates of these trends could become confounded if a path of association exists between the log slope and the true whale watch effort intensity. For example, suppose the whales do in fact spend more time in regions of high slope. Suppose also that whale watch companies learn this and preferentially situate their vessels in regions of higher slope with the intention of encountering more whales! Here, we would have a path from log depth through to the true effort. This would lead to us over-estimate the effect of log slope on the true whale intensity $\lambda_{true}$! 

Why? Because there would be an effect of log slope on the true effort (say $\alpha_1^\star$) and an effect of log slope on the true intensity ($\beta^\star_1$). By crudely estimating the whale watch effort using only a distance-from-port variable, and by incorrectly assuming that log slope **only** affects the true intensity, our model would estimate $\alpha_1^\star + \beta^\star_1$ instead of the desired $\beta^\star_1$! The addition comes from the fact that $exp(\alpha_1^\star X(s))exp(\beta^\star_1 X(s)) = exp((\alpha_1^\star + \beta^\star_1) X(s))$. 
...
From the output of model 2 we find a similar, but larger estimate for the effect of log slope. Furthermore, the estimated marginal standard deviation is estimated to be very high. This is a cause for concern, as it may be evidence that some overfitting is present which may lead to an increased variance in predictions.

Despite the **larger** DIC and evidence of overfitting, we may still prefer the results from this model over the first model. We saw before that the unconstrained model led to unrealistic predictions of the relative total amounts of effort from the two ports. Let's investigate the predicted intensity and estimated number of groups from model 2.
...
We once again restrict our predictions of the total number of groups to the smaller domain. We find large agreement between all 4 models, with the posterior mean number of groups very similar and with overlapping credible intervals. Maps of the fitted intensity also show qualitatively similar appearances. However, the second joint model has a much greater uncertainty in the predicted number of groups with a very large upper credible interval. This appears to be caused by the larger estimate of the log slope effect, combined with the larger marginal standard deviation estimated for the random field. In summary then, it appears that this second joint model has very unstable predictions of whale intensity and should not be trusted.
...
Estimates of the number of groups from joint model 3 appear to lie somewhere in-between those from joint model 1 and joint model 2. The instabilities in the predictions seen in joint model 2 appear to have been reduced.
...
Thus, based on these results, we would favour the survey-only models over the joint models. The survey-only models relied on far fewer assumptions and were far more stable (as judged by WAIC and the smaller standard deviation of the random field). Furthermore, whereas the joint model assumed a very simplistic form for the efforts from the whale watch vessels, we are more confident that our estimates of the true survey effort are accurate.

So why haven't things improved? For starters, the test dataset used for model comparison contained only 15 sightings. Thus only huge differences in model fit would likely be detected by the scoring approach. Secondly, we made strong assumptions that: the whale watch effort is fully described by the distance from port, the whale density did not change across the three months, each survey followed identical protocols with identical detection functions and the whale density in 2011 matched that in 2007-2009.
...
### Exercise {#Ex8}
Plot a map of the estimated **search effort half-normal functions** from both ports. HINT: To get the half-normal functions we want to investigate the quantities `exp(Dist_Brier_sq)` and `exp(Dist_Quoddy_sq)`.
...
It is important to note that the instability seen in some of the models used in these workshops can be attributed to the small sizes of the datasets used in these workshops combined with the crude approach used for approximating whale watch effort. The LGCP models we considered are very flexible and may require larger sample sizes.

In practice, when this general framework is applied to much larger datasets and when more care is put into estimating the efforts of opportunistic observers, these numerical instabilities will disappear. In addition, with effort properly accounted for, more complex terms can be added to the model. These include: nonlinear covariate terms, covariate interactions, temporal and seasonal effects, and spatio-temporal terms. Including such terms may substantially improve model fit. For completeness, we briefly demonstrate the code required to fit spatio-temporal models next.
...
Instead, we will model the observed whale watch intensity as a function of the estimated (half-normal) effort intensity and an estimated function of log slope **only**. That is, the whale watch encounters will be modelled as a realisation from an inhomogeneous Poisson process (IPP), while the survey data will be modelled as a realisation from a log-Gaussian Cox process (LGCP) model. The estimated `log_Slope` parameter will be shared across the two models While we do not believe the IPP to be an ideal model for the whale watch encounter locations, we hope to use it to help us get better estimates of the effect of log slope and hence better estimates of $\lambda_{true}$. 

Why might modelling the whale watch encounters with an IPP improve estimates of the log slope effect? Unlike the LGCP model, unmeasured effort covariates are allowed to exist for the whale watch data under the IPP! A weaker assumption is required instead: namely that no path of association from log slope to the true effort intensity exists. Note that the impact of assuming an incorrect inhomogeneous Poisson process model for opportunistic data sources on the width of posterior credible intervals of parameters and predictions is an open research problem. 

As a side point, if we fit a LGCP model for the whale watch data jointly with the survey data we get severe numerical instabilities. The lack of survey encounters within the whale watch regions leads to the spatial random field to not be identifiable in conjunction with the half-norm effort function! If you are interested in seeing the fitted model, run `fit_combined_extra <- readRDS('./Model_Files/fit_combined_extra.rds')`.
