...
*Fitting these joint models can take a while. For the purposes of this workshop, I have made all the pre-fitted model files available for use via the `readRDS` function.*
...
Next, lets ask inlabru to print the convergence information and to perform a fast empirical Bayes analysis.
...
<!-- We define a set of `samplers` for the two ports equal to the regions in the water that are less than 140km from port (assuming maximum of 70kmh average speed for 2 hours). -->

<!-- ```{r} -->
<!-- # gBuffer will define a circle of radius 140 around the port -->
<!-- # gIntersection will restore the coastline -->
<!-- Sampler_Quoddy <-  -->
<!--   gIntersection(Domain, -->
<!--                 gBuffer(WW_ports[WW_ports$COMPANY=='QUODDY LINK',],  -->
<!--                         width = 140) -->
<!--                 ) -->
<!-- Sampler_Brier <-  -->
<!--   gIntersection(Domain, -->
<!--                 gBuffer(WW_ports[WW_ports$COMPANY=='BRIER ISLAND WHALEWATCH',],  -->
<!--                         width = 140) -->
<!--                 ) -->
<!-- ggplot() + gg(Sampler_Brier, color='red') + gg(Sampler_Quoddy,color='blue') + gg(Domain) -->
<!-- ``` -->
...
Why? Because there would be an effect of log slope on the true effort (say $\alpha_1^\star$) and an effect of log slope on the true intensity ($\beta^\star_1$). By assuming that log slope effects only the true intensity, our model would estimate $\alpha_1^\star + \beta^\star_1$ instead of the desired $\beta^\star_1$! The addition comes from the fact that $exp(\alpha_1^\star X(s))exp(\beta^\star_1 X(s)) = exp((\alpha_1^\star + \beta^\star_1) X(s))$. We display this problematic whale watch scenario with a DAG below. It is the two causal arrows leaving log slope that cause the problem.  
...
Importantly then, depending on whether or not log slope is independent of the true effort, including log slope could either improve or harm our spatial predictions of the whale intensity! This is why we should attempt to model/emulate the true effort form the opportunistic data sources as accurately as possible. The better our approximations to the true effort intensity, the less we risk residual confounding hurting our estimates of species-environmental trends and the more justified we are with including the spatial field in the models for the opportunistic data.

A cause for concern with this joint model is that we estimate a **very large** value of the WAIC. A very large WAIC (or DIC) value can often be a sign that the model is having convergence issues (is nearly singular). This instability is consistently seen throughout the remainder of this workshop session. The instability is due to us fitting a very complex model to a very small subset of the available data (we used only 63 survey sightings!). In practice, we should not confidently report results from such a model. However, for the purposes of this workshop, we continue and report the DIC results. 
...
For our second model, we will use the available information on the fleet sizes of the two whale watch companies and the number and duration of trips that leave each port. Looking at the two websites, we estimate Brier to have 3.22 times more effort on the water than Quoddy. Additional details on computing this number are given at the end of this workshop [click here](#WW_est).

Note that the previous model estimated Brier's effort surface to be only `r signif(Rel_Effort$mean,2)` times bigger! This is unrealistic... Instead, in model 2, we will fix the total effort from Brier equal to 3.22 times that of Quoddy. To achieve this, we will replace the contrast term (`Diff_...`) with a log(3.22) offset to the Brier linear predictor. This is the reason we defined the model in terms of a contrast!

For the log offset to truly represent the relative efforts, we also need to define an integration function to integrate the whale watch effort intensities across the two respective 'sampler' polygons. This function will use the current guess of the parameters `Dist_Brier_sq` and `Dist_Quoddy_sq` and the integral it computes will be used to normalise the two effort surfaces and put them on the same scale. This will allow us to use the log(3.22) offset to represent the relative effort. Let's define these now. 
...
We now define the shared components and the formulae. Then, we define the likelihood objects and fit the joint model using `bru`. We place a Gaussian prior on the `Diff_QQ_B_Q` term in the components with the arguments `mean.linear=1.17, prec.linear=100`. Note that the precision is defined as the inverse of the variance and 1.17 = log(3.22). 
...
Estimates of the number of groups from joint model 3 appear to lie somewhere in-between those from joint model 1 and joint model 2.

We can estimate the relative effort across the two ports? Given that we have normalised the effort fields by their total integrals, we can assess the estimated relative effort by simply assessing the posterior marginal distribution of the exponential of `Diff_QQ_B_Q`.
...
# predict
Slopepred_WW1 <- sapply(samples_WW1,FUN = function(x){return(exp(Slopedf$log_Slope*x$log_Slope))})

Slopepred_WW2 <- sapply(samples_WW2,FUN = function(x){return(exp(Slopedf$log_Slope*x$log_Slope))})

Slopepred_WW3 <- sapply(samples_WW3,FUN = function(x){return(exp(Slopedf$log_Slope*x$log_Slope))})

# plot
multiplot(
ggplot(data.frame(mean=apply(Slopepred_WW1,1, mean),
                  LCL=apply(Slopepred_WW1,1, quantile, probs=c(0.025)),
                  UCL=apply(Slopepred_WW1,1, quantile, probs=c(0.975)),
                  logSlope=Slopedf$log_Slope),
       aes(y=mean,x=logSlope,ymax=UCL,ymin=LCL)) +
  geom_line() + geom_ribbon(alpha=0.4) + ylim(c(0,170)) +
  ylab('Relative intensity change') +
  ggtitle('Joint model 1'),

ggplot(data.frame(mean=apply(Slopepred_WW2,1, mean),
                  LCL=apply(Slopepred_WW2,1, quantile, probs=c(0.025)),
                  UCL=apply(Slopepred_WW2,1, quantile, probs=c(0.975)),
                  logSlope=Slopedf$log_Slope),
       aes(y=mean,x=logSlope,ymax=UCL,ymin=LCL)) +
  geom_line() + geom_ribbon(alpha=0.4) + ylim(c(0,170)) +
  ylab('Relative intensity change') +
...
ggplot(data.frame(mean=apply(Slopepred_WW3,1, mean),
                  LCL=apply(Slopepred_WW3,1, quantile, probs=c(0.025)),
                  UCL=apply(Slopepred_WW3,1, quantile, probs=c(0.975)),
                  logSlope=Slopedf$log_Slope),
       aes(y=mean,x=logSlope,ymax=UCL,ymin=LCL)) +
  geom_line() + geom_ribbon(alpha=0.4) + ylim(c(0,170)) +
  ylab('Relative intensity change') +
...
We see that all three models under-estimate the total observed number of encounters, but that all three models contain the observed value within their 95% credible intervals. Any differences between the three models is minor. 
...
What have we gained? For starters, by including the whale watch data within a joint model, we gained knowledge about a possible relationship between whale density and slope. Of course, the causal DAG introduced in the third lecture would tell us to be cautious: the detected trend could be due to confounding with terms that truly drive whale watch effort! Additionally, we now have a model for the number of sightings to be expected from the whale watch companies given an estimate of their total efforts. This could be used to detect large changes in the whale intensity within the waters in close proximity to their ports. Furthermore, our approach at approximating whale watch effort was very crude. Future work should consider emulating the whale watch efforts more accurately (e.g. obtaining 'typical' routes travelled by the vessels)..
...
Many of the numerical instabilities we faced with fitting these models may also disappear with the commercial data included for three reasons. First, unlike the whale watch data, the commercial opportunistic data covers a much larger area of the domain. Second, commercial vessels may be less prone to preferentially focusing their efforts in regions where the whale intensity is high. The DAG tells us that this should reduce the risk of over-estimating species-environment trends (due to confounding). Finally, there is more overlap between the survey effort and the commercial effort. This should help with estimating the intercept parameters.
...
### Exercise {#Ex7}
Now, predict the fitted detection function from `fit_combined4` and compare it with the detection function estimated from `fit_combined2`. Combine the two into a single plot using `multiplot`. HINT: You may need to use the `generate` and `sapply` method if an error message is returned when using `predict`.

<div class="fold s o">
```{r, cache=TRUE}
distdf <- data.frame(distance = seq(0, 2, length = 100))
dfun_3 <- generate(fit_combined3, n.samples = 20)
dfun_3 <- sapply(dfun_3, FUN = function(x){exp(log_hn(distdf$distance, as.numeric(x$lsig)))})
  
dfun_4 <- generate(fit_combined4, n.samples = 20)
dfun_4 <- sapply(dfun_4, FUN = function(x){exp(log_hr(distdf$distance, as.numeric(x$lsig)))})

dfun_df <- data.frame(distance=distdf$distance,
                      mean_hnorm=apply(dfun_3,1,mean),
                      mean_hazard=apply(dfun_4,1,mean),
                      LCL_hnorm=apply(dfun_3,1,quantile,probs=0.025),
                      UCL_hnorm=apply(dfun_3,1,quantile,probs=0.975),
                      LCL_hazard=apply(dfun_4,1,quantile,probs=0.025),
                      UCL_hazard=apply(dfun_4,1,quantile,probs=0.975)
                      )
multiplot(
...
<!-- Additional opportunistic sightings are available in the SpatialPointsDataFrame object `Sightings_Opp_sp`, obtained by running the code below. Obtain AIS vessel density data, or manually create a rough one by hand.  Try to incorporate these additional sightings into a model. What changes?  -->
...
These workshops would not have been possible without the extensive organisational and data-preparation efforts of Shelley Lang and Catalina Gomez. We thank you both! Additionally, Finally, we would like to thank Finn Lindgren for making a stable release version of `inlabru` in time for these workshops and for providing coding assistance.