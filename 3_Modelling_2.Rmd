...
As we did with depth and slope in the previous tutorial, we need to buffer and rescale these covariates.  Unlike with previous covariates, however, we will fix all bufffered values on land equal to a large constant. This will help to ensure that negligible effort is recorded from the whale watch vessels on land through $\lambda_{eff}$.

We show how to do it with the disatnce to Brier.

**JOE: please add more comments in the code. I have already forgotten the details ;).**
...
### Exercise {#ExNew}
**JOE: I have created a exercise for Quoddy, we may need to add below and renumber things**
...
### Functional form of the distance to port covariate
...
**JOE: did you do this in the exploratory analysis? Both choosing half-norm and using the sq? I think so. If so, I would changes things around. And just make it clear in the first tutorial why you are doing this and that you'll be using it in the model. Then, here in this tutorial just remind the participants that you chose the half-normal and that thus you need the sq and refer back to first tutorial. Just load directly the squared objects and use this from the get go. This model 2 tutorial is essential and is much much longer than exploratory stuff. So we want to focus on the model and remove when we can unecessary code (here code that has been run before). You coudl add non-echoed code chunks that show the historgrams again, if you worry that they will forget.**
...
**JOE: be more specific here, what are you looking at to say that we detect a large negative value**

The output from `summary` shows some interesting findings! 

We detect a large negative effect of the scaled squared distance from port. This is expected, indicating a decreased effort intensity as the distance from the port increases.

Most interestingly, we detect a large significant effect of log_Depth on the whale intensity. This is in agreement with the earlier exploratory analysis. Thus, it appears that we may have gained a substantial amount of precision by incorporating the whale-watch data into our model! This should help to improve the spatial predictions of the whale intensity too. **JOE: are you using some specific values to say that we have gained substanrial precision? Worth being specific, and maybe even reminding them of the results from the previous tutorial.**
...
**JOE: please comment the code a bit more here**
...
We see that the vessels from Brier are estimated to travel further from port compared with those from Quoddy which are estimated to stay very close to port. Furthermore, we estimate that in total, the effort from the Brier port is around ~1.5 times greater than that from the Quoddy port. This finding will be called into question later. **JOE: is that just from the plots?**

We can also look at the estimated relative total effort from the two companies Brier/Quoddy
```{r 'map-effotWW1Quoddy', message=FALSE, cache=TRUE}
Rel_Effort <-  
...
**JOE: can you interpret that result, in particular can you say what mean Rel_Effort (with units).**   
