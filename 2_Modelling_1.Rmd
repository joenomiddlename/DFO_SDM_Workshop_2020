...
<!-- Domain_restricted <- gSimplify(gBuffer(Domain,width=15),tol=5) # again all the survey tracklines are contained in the modified boundary -->
<!-- ``` -->
...
Before defining our priors, let's remind ourselves how the Gaussian random field is affected by the range and marginal standard deviation. We now show example plots, similar to those provided in the previous lecture. 

```{r, echo=F, cache=T}
# Marie - this makes the files look funny
#include_graphics('./MaternFields/Matern_SD1_R07.pdf')
...
```{r, echo=F, cache=T}
#include_graphics('./MaternFields/Matern_SD1_R07_2.pdf')
sim_range <- 0.7
sim_sd <- 1
...
Next, we must define our detection probability function. First, we define a half-normal detection probability function. This object will take the form of a base R function.
...
```{r 'bru', message=FALSE, cache=TRUE, eval=F}
# Warning: can be slow!
fit <- bru(fit, components = cmp_land1)
```

```{r 'bruload', message=FALSE, cache=TRUE, echo=F}
...
where $\hat{\bar{\lambda}}_{true}(s_i)$ are a single sample of values of ${\lambda}}_{true}$ evaluated at the locations $s_i$ defined by `ipoints`, with weights $|A_i|$. By generating a large number of Monte Carlo samples of $\hat{\bar{\lambda}}_{true}(s_i)$, we are able to compute any quantity of interest for the total group size, including the mean, standard deviation, etc.,
...
```{r 'bru2', warning=FALSE, cache=TRUE, eval=F}
# Slow to run bru
fit2 <- bru(fit2, components = cmp_land2)
```
```{r 'bruload2', message=FALSE, cache=TRUE, echo=F}
...
If the predict function breaks, use the code below:
<div class="fold s o"> 
```{r}
