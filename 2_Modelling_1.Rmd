...
Before defining our priors, let's remind ourselves how the Gaussian random field is affected by the range and marginal standard deviation. Below is a chunk of code that simulates and plots a single Gaussian random field on the unit square. You are asked to specify the range and marginal standard deviation:
...
We are asked to define the following two probabilistic statements:
...
The population estimated is approximately `r round(Lambda$mean)` individuals with a large variance of approximately `r round(Lambda$var)`.
...
You should get a smaller population estimate of approximately `r round(Lambda_restricted$mean)` individuals and a smaller, but still large, variance of approximately `r round(Lambda_restricted$var)`.

## Adding covariates to an `inlabru` model

Adding environmental covariates to models is easy in `inlabru`. All that is required is that each covariate is converted to class SpatialPixelsDataFrame and that the covariate data are complete. By 'complete', we mean that a non-missing value is available at every point within the region defined by the mesh boundary.
...
<div class="fold s o"> 
```{r 'cmp_land4', warning=FALSE, cache=TRUE}
...
<div class="fold s o"> 
```{r 'like4', warning=FALSE, cache=TRUE}
...
<div class="fold s o"> 
```{r 'cmp_land_extra', warning=FALSE, cache=TRUE}
...
<div class="fold s o"> 
```{r 'likeExtra', warning=FALSE, cache=TRUE}
