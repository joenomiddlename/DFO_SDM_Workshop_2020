...
We now define our model, with $\rho_0$, $p_{\rho}$, $\sigma_0$, and $p_{\sigma}$ fixed at 35km, 0.01, 2, and 0.01 respectively. Roughly speaking, our priors state that the range is likely larger than 35km and the standard deviation is smaller than 2. Why 2? A mean-zero Gaussian random field with standard deviation 2 can be expected to have approximately 95% of its values between $\pm 4$ (due to the normal distribution's $\pm 2 \sigma$-rule). On the multiplicative (exponential) scale of our model, this allows for a range of whale intensity values of $\textrm{exp}(\pm 4)$. Such a large range of values allows for the existence of regions $A$ and $B$ in $\Omega$ such that the whale intensity in $A$ is around 3000 times greater than the intensity in $B$! We do not believe that relative differences greater than this are probable.
