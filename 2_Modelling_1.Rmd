...
The population estimated is approximately `r round(Lambda$mean)` individuals with a large variance of approximately `r signif(Lambda$var,2)`. 
...
You should get a smaller population estimate of approximately `r signif(Lambda_restricted$mean,1)` individuals and a smaller, but still large, variance of approximately `r signif(Lambda_restricted$var,1)`. Don't worry if your answers are slightly different - we've only used 20 samples!
...
So, we find that adding log_Depth to the model improves the model fit, as judged by DIC and WAIC. In the paper that developed the DIC method, Spiegelhalter et al (2002) had the following to say:
...
<!-- spTransform(Domain_restricted,CRS('+proj=longlat +datum=WGS84 +no_defs'))@bbox -->
