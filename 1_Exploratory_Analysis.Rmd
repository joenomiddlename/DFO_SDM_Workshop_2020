...
We can then map the distance to the nearest trackline. **JOE: my understanding from teh description in the previous paragraph, though I find the code above a bit confusing, is that it is combining the distance to port to the survey line distance. I'm confused why the below only shows the distance to trackline.**
...
**JOE: is was a bit confusing when lumped with the above, at first sight it felt like the historgram was for the nearest distance you calculated in the above. I think the detection function stuff could likely have it own section further above.**

To explore which detection function should be used, we now plot the histogram of measures distances in the survey data (i.e. the observed distances of sightings from the trackline).
...
The histogram perhaps shows an initial steady decline in sightings with distance, before decaying more quickly as distance increases. Again, we need to remember that this plot is showing associations between distance $d$ and $\lambda_{obs}$ and **not** with $p(d)$. **JOE: you need to explain this, I don't understand why it's not something close to $p(d)$. I'm not sure when you mentioned this (the 'again' above implies that we have talked about this before)? In the lecture? ** To capture this shape, we will use a half-norm detection function: 

$$p(d) = exp\left(\frac{-d^2}{2\sigma^2}\right)$$ 
This requires us to square the values of distance from the trackline.

```{r 'Surbeylines-halfnorm', cache=TRUE}
Dist_Surveylines$Dist_surveylines <- scale(Dist_Surveylines$Dist_surveylines^2)
```

**JOE: I would make a different section for this too before the ppm stuff. I would present both as, remember we are interested in p(d) and so we look into it. And here for distance to port I would say, remember from lecture 1, when we don't have effort data we can model it with covariates. Here we will use for whale-watch data, distance from port. I would push all of the ppm stuff further below after having talked about the main parts described in lecture 1 and that will be used in your models in the next.**
