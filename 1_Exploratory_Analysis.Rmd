...
Planned surveys can be designed to (approximately) satisfy the independence assumption. The plane speed can be fixed at a high value relative to the individuals' speeds (no double-counting) and a narrow perpendicular field-of-view can be enforced. Then, assuming that the individuals of the target species are in equilibrium with respect to their stationary distribution $\lambda_{true}(\textbf{s})$ (i.e. the species' density) and locally mixing faster than the time between revisits by observers, the independence assumption may reasonably hold.
...
Below, we can see that there is no overlap between the sighting locations from the two whale-watch companies, suggesting that an assumption of independent encounters between the two companies may be reasonable. 
...
However, the distance between the average locations of the encounters made by the two companies is around 80km:
...
Given the speed at which fin whales travel, we should investigate this further in a real analysis, especially since a violation of this assumption will results in overestimating the densities of whales in the whale-watching areas. 
...
These two plots also show evidence of potential autocorrelation existing up to ~14 days. This could be due to persistence of animal movement, for example, due to a few whales remaining in the same area for a few days. 
...
**JOE: this section needs work. I know you now changed to inlabru and that's great, but I had more or less removed lost of details from this section before so they would addressed above to  help discuss things in small manageable packages (you can still see this from my old comment above), but you have re-put a lot of the details back. I must be honest this is very discouraging to me. Your work is great and the inlabru change here is also a great improvement on using ppm, and I know you worked days and day on this. But I spend hours (evening hours) doing these changes to help make the material more understandable for ecologists, and I feel they were more less ignored and reverted back (or not merged). It's been a bit of a patterns, so it's particularly hard for me to take it now. It's particularly weird now, because you don't go into the details of inlabru, but yet give tons of details, many of them with the preface of we are going to talk about this again, or more, or. Just talk about things well up front, rather than repeating yourself. I also thing this is a missed oppotunity to present some of the ideas of inlabru syntax and I'm not sure what's the point of this now, since it's a lot of "we will talk about this later". What do you want them to get out of this? I've spent again some time doing the changes, but please read this critically, and think about whether you have said this before. If so cut it, it there is just one additional point, move that additional point in the main place you talk about it. **
...
Remember that $N(Y \cap A)$ represents the number of encounters in $A$. We will see in lecture 2 that this equation defines an inhomogeneous Poisson process, where we assume that the rate of encounters with whales changes through space according (only) to a set of measured covariates (i.e., the rate is spatially-inhomogeneous). The above distribution for $N(Y \cap A)$ will hold for any number of non-overlapping regions $A_i \subset \Omega$. If we define a grid of non-overlapping cells $A_i$ that cover the domain $\Omega$, then as the number of cells of the grid increases, the (Poisson) likelihood of the counts (falling within each $A_i$) will converge to the likelihood of the inhomogeneous Poisson process that preserves the exact locations of the encounters! Do not panic if this point is unclear - we will cover it in depth in lecture 2.

The `inlabru` package can fit inhomogeneous Poisson processes to the data using a similar approximation technique to the one defined above! Thus, we can use `inlabru`to investigate trends between the observed intensity and the log slope covariate using the encounter data. 

In the next 2 tutorials, we will explore `inlabru`'s ability to fit both inhomogeneous Poisson processes and log-Gaussian Cox processes and we will learn the syntax in depth. For now, we will look at the output from `inlabru` models for the purposes of exploratory analyses.
...
**JOE: this is a repeat, I'm pretty sure I had moved most of that up before.**
...
\lambda_{eff,i}(s) = \lambda_i exp\left(\frac{-||s-s_i^*||^2}{2\sigma_i^2} \right) \textrm{ for shale-watch data} \\  
...
**JOE: Stating again that things will be talked about later is not particularly useful and a bit of a wasted opportunity to explain things. **
