...
**JOE: we need to define all of the terms used later in the tutorial, e.g., $\lambda_{eff}$. Depending on the lecture, you can be or less technical. But they should all be described at least intuitively here up-front**
...
This hypothesis would imply a traveling speed of between 4km/h and 16km/h. Is this plausible? Since fin whales are known for their speed and can sustain much higher speeds (> 30km/h), we will discard all repeated sightings each day. We will keep only the first sighting made each day by each company. 
...
To distances against time intervals, we first compute the distances in time and the euclidean distances in space between each sighting. 
...
The third plot investigate the space-time relationship across years.
...
**JOE: this is likely the same whales staying in the area for a few days. I think you'll have to address this at some point, either here or later when talking about the modeling. It would be good to acknowledged here that this may be the case.**

### Creating training and testing datasets

**JOE: add one line about why you need a training and test dataset.**

We now subset the data into a training and test set. We choose the training set to contain all the sightings from 2007-2009. For the test data, we choose **all** the sightings from 2011. See [Additional tips](#Ad4) for an explanation of why the potential repeated sighting in 2011 between survey and whale-watch datat, means that the whale-watching data cannot be added to the training dataset.
...
Next, we evaluate a crude estimate of the total amount of survey effort spent each year, as measured by the total trackline length. Using this, we then crudely estimate a 'catch per unit effort' (CPU) measure by year.
...
**JOE: I have removed the plots and code that were similar to the survey plots at the very beginning, but we may want to think about merging the above section with this one and reordering things to reduce repeating things and to make the flow easier.**

## Investigating trends between the environmental covariates and whale density

We will investigate trends between the whale density and the covariates bathymetry and slope. 
...
# Notice how easy it is to extract the values of slope and bathymetry at these locations!
Slope_obs <- Slope_im[All_obs_ppp]
Bathym_obs <- Bathym_im[All_obs_ppp]
```

First, we plot the empirical densities of the slope at the sightings locations at throughout the domain (study area). 
...
**JOE: add a comment in text here, and talk about the log transform rather than in code. It's just more digestable**

Perhaps a log transform is desirable here to make the covariate more 'normal'.

```{r 'map-slopeLog', message=FALSE, warning=FALSE, cache=TRUE}
ggplot(data=data.frame(Slope_obs = log(Slope_obs)),
       aes(x=Slope_obs, colour='Sightings')) +
  geom_density() +
  geom_density(data=data.frame(Slope_dom = log(as.numeric(Slope_im$v))),
               aes(x=Slope_dom, colour='Domain')) + xlab('log Slope') +
  scale_color_manual(name = "Density", values = c('Sightings' = 'black', 'Domain' = 'red'))
```

Perhaps there is evidence of whales preferring areas of higher slope?

We can do the same with bathymetry, but let's only consider values in water (max bathym is 313, so restrict to < 0).
...
Perhaps a log transform is desired here too and let's flip the sign to make it depth (larger is deeper).
...
These plots may suggests that sightings locations are associated with regions with flatter slopes and shallower waters compared what is available in the study area. However, these results may simply reflect the types of regions preferentially visited by observers!

We could go one step further and fit Poisson point process models to the sightings data using the `ppm()` function from the spatstat package. These can be equivalent to Maxent models. Ignoring effort for the time being, what does a Poisson point process model tell us?
...
Ppm_1 <- ppm(All_obs_ppp~log_Slope_im + log_Bathym_im,
...
We find some evidence that the whales may be found more often in regions of higher slope and shallower waters.

**JOE: be specific here, base on the coefficient, Ztest, etc? Some people will know these models, some won't at all.**
...
Based on the histogram (**JOE: maybe add characteristics that made you choose this in brackets**), we will use a half-norm detection function in the Poisson point process model. Thus, we will square the values.
...
Now we can fit the Poisson point process models to the **JOE: you need a bit more info here.**.

```{r 'ppm2', warning=FALSE, message=FALSE, cache=TRUE}
Ppm_2 <- ppm(All_obs_ppp~log_Slope_im+log_Bathym_im +
...
This is why we turn to log-Gaussian Cox processes in the next tutorial. By assuming the intensity surface to be a realisation from a spatially-smooth (log-Gaussian) random field, we are able to adjust our inference to take into account any additional spatial correlations and/or unmeasured covariates! By doing this, both parameter estimates and measures of uncertainty should be better adjusted to account for these autocorrelations.
